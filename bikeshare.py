...
''')
    ...
    df['combination'] = df['Start Station'] + ' ' + 'to' + ' ' \
        + df['End Station']
...
''')
    ...
    print ('The total trip duration: {} hour(s) {} minute(s) {} second(s)'.format(hour,
            minute, second))
    ...
    if m > 60:
        (h, m) = divmod(m, 60)
        print ('The total trip duration: {} hour(s) {} minute(s) {} second(s)'.format(h,
                m, sec))
    else:
        print ('The total trip duration: {} minute(s) {} second(s)'.format(m,
                sec))
...
def display_data(df):
    ...
    while True:
        response = ['yes', 'no']
        choice = \
            input("Would you like to view individual trip data (5 entries)? Type 'yes' or 'no'\n"
                  ).lower()
    ...
    if choice == 'yes':
        while True:
            choice_2 = \
                input("Would you like to view more trip data? Type 'yes' or 'no'\n"
                      ).lower()
